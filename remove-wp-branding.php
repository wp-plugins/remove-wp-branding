...
Plugin URI: http://workshop-mg.com/remove-wordpress-branding/
...
function rwp_initial_install() {
	...
	add_option( 'rwp_eliminate_logos', '1' );
...
//If statement to hide admin bar wordpress logo and help menu
if (get_option('rwp_eliminate_logos') == 1){
...
add_action('admin_menu', 'rwp_add_pages');
	...
	function custom_howdy_settings() {
		...
		register_setting( 'rwp-custom-howdy-group', 'rwp_eliminate_logos' );
...
function rwp_add_media_admin_styles() {
wp_enqueue_style('thickbox');
}

add_action('admin_print_scripts', 'rwp_add_media_admin_scripts');
add_action('admin_print_styles', 'rwp_add_media_admin_styles');
//Add media upload scripts for logo page upload end
//this needs adjustment - it gives too many options.
    ...
    <div style="clear:both;"></div>
		...
		<form method="post" action="options.php">
		    ...
		    <table class="form-table">
		        ...
		        <?php if (get_option('rwp_eliminate_logos') == 1){$chkysno = 'checked';} else {$chkysno = '';}?>
		        <td><input type="checkbox" name="rwp_eliminate_logos" value="1" <?php echo $chkysno; ?> />*This hides Wordpress logo upper left hand corner and cleans the standard menus out of the admin bar.</td>
	        	</tr>
				...
				jQuery('#rwp_upload_image_button').click(function() {
				formfield = jQuery('#rwp_upload_image').attr('name');
				...
				jQuery('#rwp_upload_image').val(imgurl);
				tb_remove();
				}
